...
for (i in 1:length(Train_Idx)){
	nameFoldTrain = paste(as.character(i),"_crossValidationTrain.tsv",sep="")
	nameFoldTest = paste(as.character(i),"_crossValidationTest.tsv",sep="")
	dfTrain <- data.frame(train=Train_Idx[[i]]-1)
	dfTest <- data.frame(test=Test_Idx[[i]]-1)
	write.table(dfTrain,nameFoldTrain,sep="\t",quote = FALSE)
	write.table(dfTest,nameFoldTest,sep="\t",quote = FALSE)
}
