...
if __name__ == "__main__":
    ...
    inp = sys.argv[1]   #directory path
    if inp == "pbmc":
        ls = os.listdir("INTER_benchmark/pbmc/")
        for e in ls:
            path = "INTER_benchmark/pbmc/"+e
            cv_folds = os.listdir(path)
            cv_folds.remove("AnnolistsBuilder_results")
            cv_folds.remove("custom")
        ...
        ls = os.listdir(inp)
        for cv in ls:
            path_original = inp+"/"+cv+"/benchmark_labels.tsv"
            path_annotated = inp+"/"+cv+"/results_directory/p_values.tsv"
            path_deltas = inp+"/"+cv+"/results_directory/deltas.tsv"
            comparison = annotationComparator(path_annotated,path_original,path_deltas)
            collected_original = comparison[0]
            collected_annotated = comparison[1]
            DFO = pd.DataFrame.from_dict({"original":collected_original})
            ...
            os.system("Rscript --vanilla evaluate.r "+inp+" "+cv)
            os.system("mv originalAnno.csv scaltAnno.csv "+inp+"/"+cv)
