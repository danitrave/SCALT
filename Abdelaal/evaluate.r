...
evaluate <- function(TrueLabelsPath, PredLabelsPath, Indices = NULL){
  ...
  conf_F1 <- table(true_lab,pred_lab,exclude = c('unassigned','Unassigned','Unknown','rand','Node','ambiguous','unknown',"multiassigned"))
  ...
  F1[F1==0.0]<-NA 
  med_F1 <- median(F1,na.rm = TRUE)
  
  total <- length(pred_lab)
  num_unlab <- sum(pred_lab=="multiassigned") +sum(pred_lab == 'unassigned') + sum(pred_lab == 'Unassigned') + sum(pred_lab == 'rand') + sum(pred_lab == 'Unknown') + sum(pred_lab == 'unknown') + sum(pred_lab == 'Node') + sum(pred_lab == 'ambiguous')
  per_unlab <- num_unlab / total
  
  acc <- sum_acc/sum(conf_F1)
  
  result <- list(Conf = conf, MedF1 = med_F1, F1 = F1, Acc = acc, PercUnl = per_unlab)
  #result <- list(PercUnl = per_unlab)
  return(result)
}
train = args[1]
test = args[2]
print(train)
print(test)
print(evaluate("originalAnno.csv","scaltAnno.csv"))
