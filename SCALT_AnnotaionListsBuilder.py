...
def lists_generator(counts,anno,boo,cells,genes,notation,cpus):
    ...
    try:
        os.system("python3 scripts/inputPreparation.py "+counts+" "+anno+" anno "+notation)   #input adjustment 
    except:
    ...
    try:
        os.system("mv boostraps_samples/ cellTypes_fromAnnotationHeatmap.png genes2remove.tsv genesCellTypes_probabilities.tsv genes_entropy.tsv genesGeneral_probabilities.tsv genesProbabilities_ratios.tsv genesRanking.tsv groupped_cell_types/ TABLE_OF_GENES.tsv "+anno+" AnnolistsBuilder_results/")
    except:
